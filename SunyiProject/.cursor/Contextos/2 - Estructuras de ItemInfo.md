...
* #### **`STR_IntentionActions`**:
    * **Descripción:** Define un vínculo entre una **acción de intención** (ej. `Action.Container.Equip.Wield`, `Action.Container.Equip.Ammo`) y los **slots candidatos** donde esa intención debe intentar equipar. Se usa para que el orquestador determine el slot correcto según el contexto, sin hardcodear tags en lógica.
    * **`ActionTag`** (`FGameplayTag`): Tag de la intención/acción que disparó el flujo.
    * **`SlotCandidates`** (`TArray<FGameplayTag>`): Lista **ordenada** de slots candidatos (ej. `Equipment.FreeHandRight` → `Equipment.FreeHandLeft`), que define prioridad al equipar.
...
* #### **`STR_EquipmentBehavior`**:
    ...
    * **`EquippableSlots`** (`FGameplayTagContainer`): Slots `Equipment.*` donde este ítem **puede** equiparse (superset). La validación final decide el slot real según contexto (lateralidad, intención, UI drag, etc.).
    * **`AllowedCreature`** (`FGameplayTagContainer`): Roots `Character.*` de criaturas que pueden equipar este ítem (ej. `Character.Human`).
    * **`ActionBindings`** (`TArray<STR_IntentionActions>`): Mapeo de **intención → slots candidatos**. Permite que el orquestador, dado un `ActionTag`, obtenga los slots candidatos correctos para equipar (ej. roca como arma vs como munición).
